      ...
      <div className="p-4 pt-10 flex-1 flex flex-col">
        ...
        <div className={`p-3 rounded-xl mb-3 ${isDark ? 'bg-white/5' : 'bg-gray-50'}`}>
          ...
          {breakdown?.nextDateReset && (
            <div className={`text-xs ${colors.textMuted} mt-1 flex items-center gap-1`}>
              <Clock size={10} />
              {new Date(breakdown.nextDateReset * 1000).toLocaleDateString('zh-CN', { year: 'numeric', month: '2-digit', day: '2-digit' })} {t('common.reset')}
            </div>
          )}
        </div>
        ...
        {(() => {
          ...
          if (freeTrialExpiry && freeTrialExpiry > Date.now() / 1000) {
            ...
            return (
              <div className={`text-xs mb-3 flex items-center gap-1 ${daysLeft <= 3 ? 'text-orange-500 font-medium' : colors.textMuted}`}>
                <Clock size={12} />
                试用剩余 {daysLeft} 天 ({expiryDate.toLocaleDateString('zh-CN', { year: 'numeric', month: '2-digit', day: '2-digit' })})
              </div>
            )
          }
          ...
          if (nextDateReset) {
            ...
            return (
              <div className={`text-xs ${colors.textMuted} mb-3 flex items-center gap-1`}>
                <Clock size={12} />
                {daysLeft} 天后重置 ({resetDate.toLocaleDateString('zh-CN', { year: 'numeric', month: '2-digit', day: '2-digit' })})
              </div>
            )
          }
