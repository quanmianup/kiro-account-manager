import { Search, Download, Upload, RefreshCw, Trash2, Plus, Sparkles, LayoutGrid, List } from 'lucide-react'
    ...
    <div className={`${colors.card} border-b ${colors.cardBorder} px-6 py-4 relative overflow-hidden`}>
      ...
      <div className="flex items-center justify-between relative">
        ...
        <div className="flex items-center gap-3 animate-fade-in delay-400">
          ...
          <div className={`flex items-center border ${colors.cardBorder} rounded-xl overflow-hidden`}>
            <button 
              onClick={() => onViewModeChange('list')} 
              className={`btn-icon px-3 py-2 transition-all ${viewMode === 'list' ? (isDark ? 'bg-blue-500/20 text-blue-400' : 'bg-blue-100 text-blue-600') : (isDark ? 'hover:bg-white/5' : 'hover:bg-gray-50')}`} 
              title="列表视图"
            >
              <List size={16} className={viewMode === 'list' ? '' : colors.textMuted} />
            </button>
            <div className={`w-px h-4 ${colors.cardBorder.split(' ')[1]}`} style={{ backgroundColor: isDark ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.1)' }} />
            <button 
              onClick={() => onViewModeChange('grid')} 
              className={`btn-icon px-3 py-2 transition-all ${viewMode === 'grid' ? (isDark ? 'bg-blue-500/20 text-blue-400' : 'bg-blue-100 text-blue-600') : (isDark ? 'hover:bg-white/5' : 'hover:bg-gray-50')}`} 
              title="网格视图"
            >
              <LayoutGrid size={16} className={viewMode === 'grid' ? '' : colors.textMuted} />
            </button>
          </div>
          <button 
            onClick={onRefreshAll} 
            disabled={autoRefreshing} 
            className={`btn-icon px-3 py-2 ${colors.card} border ${colors.cardBorder} rounded-xl ${isDark ? 'hover:bg-white/5' : 'hover:bg-gray-50'} disabled:opacity-50 transition-all`} 
