    ...
    <div className={`h-full overflow-auto ${colors.main}`}>
      ...
      <div className="max-w-5xl mx-auto p-8 relative">
          ...
          <div className={`card-glow ${colors.card} rounded-2xl shadow-sm border ${colors.cardBorder} overflow-hidden animate-scale-in delay-400`}>
            ...
            <div className="p-5 space-y-4">
              ...
              <div className={`${isDark ? 'bg-gradient-to-r from-emerald-500/10 to-blue-500/10' : 'bg-gradient-to-r from-emerald-50 to-blue-50'} rounded-xl p-4`}>
                <div className="flex items-center justify-between mb-3">
                  <span className={`text-sm font-medium ${colors.text}`}>总体使用率</span>
                  <span className={`text-2xl font-bold ${
                    stats.usagePercent > 80 ? 'text-red-500' : 
                    stats.usagePercent > 50 ? 'text-amber-500' : 
                    'text-emerald-500'
                  }`}>{stats.usagePercent}%</span>
                </div>
                <div className={`h-3 ${isDark ? 'bg-white/10' : 'bg-white'} rounded-full overflow-hidden shadow-inner mb-2`}>
                  <div 
                    className={`h-full rounded-full transition-all duration-1000 ease-out ${
                      stats.usagePercent > 80 ? 'bg-gradient-to-r from-red-400 to-red-500' : 
                      stats.usagePercent > 50 ? 'bg-gradient-to-r from-amber-400 to-orange-500' : 
                ...
                <div className="flex items-center justify-between text-xs">
                  <span className={colors.textMuted}>已使用 {stats.totalUsed}</span>
                  <span className={colors.textMuted}>剩余 {stats.remaining}</span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className={`${isDark ? 'bg-white/5' : 'bg-gray-50'} rounded-lg p-3`}>
                  <div className={`text-xs ${colors.textMuted} mb-1`}>总配额</div>
                  <div className={`text-2xl font-bold ${colors.text}`}>{stats.totalQuota}</div>
                </div>

                <div className={`${isDark ? 'bg-white/5' : 'bg-gray-50'} rounded-lg p-3`}>
                  <div className={`text-xs ${colors.textMuted} mb-1`}>可用配额</div>
                  <div className={`text-2xl font-bold text-emerald-500`}>{stats.remaining}</div>
                </div>
              </div>
              ...
              <div className={`${isDark ? 'bg-white/5' : 'bg-gray-50'} rounded-lg p-3`}>
                <div className="flex items-center justify-between mb-2">
                  <span className={`text-xs ${colors.textMuted}`}>账号状态</span>
                  <span className={`text-xs font-semibold ${colors.text}`}>{stats.active}/{stats.total}</span>
                </div>
        ...
        {localToken && currentAccount && (() => {
          ...
          const nextDateReset = breakdown?.nextDateReset || usageData?.nextDateReset
          // 计算距离重置的天数
          const daysUntilReset = nextDateReset 
            ? Math.max(0, Math.ceil((nextDateReset * 1000 - Date.now()) / (1000 * 60 * 60 * 24)))
                  ...
                  <div className={`${isDark ? 'bg-white/5' : 'bg-gray-50'} rounded-lg p-3`}>
                    ...
                    <div className="space-y-1.5 text-xs">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className={colors.text}>{nextDateReset ? new Date(nextDateReset * 1000).toLocaleDateString('zh-CN', { year: 'numeric', month: '2-digit', day: '2-digit' }) : '-'}</span>
                      </div>
