...
fn get_kiro_data_dir() -> Option<std::path::PathBuf> {
    #[cfg(target_os = "windows")]
    {
        std::env::var("APPDATA")
            .ok()
            .map(|p| std::path::PathBuf::from(p).join("Kiro"))
    }
    #[cfg(target_os = "macos")]
    {
...
fn reset_kiro_machine_id_inner() -> Result<KiroTelemetryInfo, String> {
    let kiro_dir = get_kiro_data_dir().ok_or("Cannot find Kiro data directory")?;
