...
fn save_app_settings_inner(updates: AppSettings) -> Result<(), String> {
    ...
    if updates.lock_model.is_some() {
        current.lock_model = updates.lock_model;
    }
    if updates.locked_model.is_some() {
        current.locked_model = updates.locked_model;
    }
    if updates.auto_refresh.is_some() {
        current.auto_refresh = updates.auto_refresh;
    }
    if updates.auto_refresh_interval.is_some() {
        current.auto_refresh_interval = updates.auto_refresh_interval;
    }
    if updates.auto_change_machine_id.is_some() {
        current.auto_change_machine_id = updates.auto_change_machine_id;
    }
    if updates.browser_path.is_some() {
        current.browser_path = updates.browser_path;
    }
    if updates.bind_machine_id_to_account.is_some() {
        current.bind_machine_id_to_account = updates.bind_machine_id_to_account;
    }
    if updates.use_bound_machine_id.is_some() {
        current.use_bound_machine_id = updates.use_bound_machine_id;
    }
    if updates.account_machine_ids.is_some() {
        current.account_machine_ids = updates.account_machine_ids;
    }

    let content =
        serde_json::to_string_pretty(&current).map_err(|e| format!("序列化失败: {}", e))?;
