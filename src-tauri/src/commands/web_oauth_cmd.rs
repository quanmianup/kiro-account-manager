...
use crate::auth::User;
use crate::providers::web::{WebOAuthInitResult, WebOAuthProvider};
...
pub async fn web_oauth_initiate(provider: String) -> Result<WebOAuthInitResponse, String> {
    ...
    if provider != "Google" && provider != "Github" {
        return Err(format!(
            "Unsupported provider: {}. Use 'Google' or 'Github'",
            provider
        ));
    }
...
pub async fn web_oauth_complete(
    ...
    println!(
        "[WebOAuth] web_oauth_complete: callback_url={}",
        &callback_url[..80.min(callback_url.len())]
    );
    ...
    let csrf_token = auth_result
        .csrf_token
        .as_ref()
        .ok_or("No csrf_token from ExchangeToken")?;
    let refresh_token = &auth_result.refresh_token;

    let portal_client = crate::providers::web::KiroWebPortalClient::new();
    let user_info = portal_client
        .get_user_info(
    ...
    let usage: Option<crate::providers::web::UserUsage> = portal_client
    ...
    println!(
        "[WebOAuth] LOGIN SUCCESS: email={}, provider={}",
...
pub async fn web_oauth_refresh(
    ...
    let refresh_token = account
        .refresh_token
        .as_ref()
        .ok_or("No refresh_token found")?;
    let web_provider = WebOAuthProvider::new(provider);
    let auth_result = web_provider
        .refresh_token_impl(access_token, csrf_token, refresh_token)
        .await?;
...
pub async fn web_oauth_login(
    ...
    *get_pending_login().lock().unwrap() = Some(init_result.clone());
    println!(
        "Saved init_result to PENDING_LOGIN, state: {}",
        init_result.state
    );
...
#[tauri::command]
pub fn web_oauth_close_window(app_handle: AppHandle, window_label: String) -> Result<(), String> {
