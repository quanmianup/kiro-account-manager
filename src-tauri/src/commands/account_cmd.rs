...
use crate::auth::{get_usage_limits_desktop, refresh_token_desktop, User};
...
pub async fn sync_account(state: State<'_, AppState>, id: String) -> Result<Account, String> {
    ...
    let (
        ...
        let idc_provider = IdcProvider::new(
            "BuilderId",
            metadata.region.as_deref().unwrap_or("us-east-1"),
            None,
        );
        let auth_result = idc_provider
            .refresh_token(refresh_token_str, metadata)
            .await?;
        (
        ...
        let auth_result = social_provider
            .refresh_token(refresh_token_str, metadata)
            .await?;
        (
            auth_result.access_token,
    ...
    let (usage_data, is_banned): (serde_json::Value, bool) = if provider_str == "BuilderId" {
        ...
        let (usage, banned) = match &usage_call {
            Ok(u) => {
                // 打印响应 JSON
                println!(
                    "[sync_account] Usage response JSON: {}",
...
pub async fn verify_account(
    ...
    let (new_access_token, new_refresh_token, quota, used, subscription_type) = if is_idc {
        ...
        let cw_client = CodeWhispererClient::new(&machine_id);
        let usage = cw_client
            .get_usage_limits(&auth_result.access_token)
            .await?;
        ...
        (
            ...
            q,
            u,
            usage.subscription_info.and_then(|s| s.subscription_type),
        )
    } else {
...
pub async fn add_account_by_social(
    ...
    let (usage_result, ban_reason) = match &usage_call {
        ...
        Err(e) if e.starts_with("BANNED:") => (
            None,
            Some(e.strip_prefix("BANNED:").unwrap_or("UNKNOWN").to_string()),
        ),
        Err(_) => (None, None),
    };
    ...
    let account = if let Some(existing) = store
        .accounts
        .iter_mut()
        .find(|a| a.email == email && a.provider.as_deref() == Some(&idp))
    {
...
pub async fn add_local_kiro_account(state: State<'_, AppState>) -> Result<Account, String> {
    ...
    let local_token =
        get_kiro_local_token().ok_or("未找到本地 Kiro 账号，请先在 Kiro IDE 中登录")?;
    ...
    if auth_method == "IdC" {
        ...
        let client_reg =
            get_client_registration(&hash).ok_or(format!("未找到客户端注册信息: {}.json", hash))?;
        ...
        add_account_by_social(state, refresh_token, Some(provider)).await
    }
}
